...
- **Paso 1:** Verifica si el `id_aportante` existe en Supabase local.
- **Paso 2:** Si no existe o es Sync Manual, llama a Wolkvox Contactos. Guarda `razon_social`, `estado_contacto`, y `carpeta_cliente` en `m_aportantes`.
...
- **Capa 1:** Server-Side Lock (`SELECT` a `t_cierres_nomina` por `id_contrato`). Si existe, lanza HTTP 403.
- **Capa 2:** Inserta/Actualiza en `t_novedades`. NO toca `m_aportantes` ni `m_empleados`.
- **Capa 3 (ReportLab):** Genera PDF mapeando propiedades como `empleado.cargo`.

## 4. MAPA DE BASE DE DATOS (SUPABASE)

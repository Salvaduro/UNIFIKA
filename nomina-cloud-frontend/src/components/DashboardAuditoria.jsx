...
export default function DashboardAuditoria() {
  ...
  // Estados para búsqueda (filtrado) y ordenamiento (sorting) en el cliente
  ...
  // 1. Filtrado en tiempo real en cualquier columna y JSON de detalles
  // 2. Ordenamiento en el cliente usando las funciones nativas de JavaScript (.filter(), .sort())
  const filteredAndSortedLogs = logs
    .filter((log) => {
      if (!searchTerm.trim()) return true;
      const term = searchTerm.toLowerCase().trim();
      const email = String(log.usuario_email || "").toLowerCase();
      const action = String(log.tipo_accion || "").toLowerCase();
      const entity = String(log.entidad_afectada || "").toLowerCase();
      const role = String(log.rol_usuario || "").toLowerCase();
      const aportante = String(log.id_aportante || "").toLowerCase();
      const detailsStr = log.detalles
        ? typeof log.detalles === "string"
          ? log.detalles.toLowerCase()
          : JSON.stringify(log.detalles).toLowerCase()
        : "";
    ...
    return (
      <span className="inline-flex ml-1.5 items-center">
        {isActive ? (
          ...
          <svg
            className="w-3.5 h-3.5 text-white/40 opacity-0 group-hover:opacity-100 transition-opacity"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
  ...
  return (
    <div className="space-y-6 mt-4 pb-8">
      ...
      {/* Barra de Búsqueda y Estadísticas de Filtrado */}
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-4">
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Buscar por email, rol, acción, contrato o contenido en JSON..."
            className="w-full pl-10 pr-10 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-[#5b97a9]/30 focus:border-[#5b97a9] transition-all"
          />
          {searchTerm && (
            <button
        ...
        <div className="text-xs font-semibold text-slate-500 whitespace-nowrap px-2">
          Mostrando <span className="text-[#5b97a9] font-bold">{filteredAndSortedLogs.length}</span> de <span className="text-slate-700 font-bold">{logs.length}</span> registro(s)
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-[#5b97a9] text-white font-bold text-xs uppercase tracking-wider select-none">
                  <th
                    onClick={() => handleSort("fecha_evento")}
                    className="py-3 px-4 cursor-pointer hover:bg-[#4a7e8d] transition-colors group whitespace-nowrap"
                  >
                    <div className="flex items-center">
                      <span>Fecha / Hora</span>
                      {renderSortArrow("fecha_evento")}
                    </div>
                  </th>
                  <th
                  ...
                  >
                    <div className="flex items-center">
                      <span>Rol</span>
                      {renderSortArrow("rol_usuario")}
                    </div>
                  </th>
                  <th
                    onClick={() => handleSort("tipo_accion")}
                    className="py-3 px-4 cursor-pointer hover:bg-[#4a7e8d] transition-colors group whitespace-nowrap"
                  >
                    <div className="flex items-center">
                      <span>Acción</span>
                      {renderSortArrow("tipo_accion")}
                    </div>
                  </th>
                  <th
                    onClick={() => handleSort("entidad_afectada")}
                    className="py-3 px-4 cursor-pointer hover:bg-[#4a7e8d] transition-colors group whitespace-nowrap"
                  >
                    <div className="flex items-center">
                      <span>Entidad</span>
                      {renderSortArrow("entidad_afectada")}
                    </div>
                  </th>
                  <th className="py-3 px-4 text-center whitespace-nowrap">Detalles</th>
                </tr>
              </thead>
